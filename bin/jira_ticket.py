...
Typical use:
  jira_ticket setup --site https://turniere.atlassian.net --email you@example.com --auth-mode scoped --cloud-id <cloud-id>
  jira_ticket PLAN-83
...
def store_token_keychain(service: str, account: str, token: str) -> None:
    ...
    # `security` does not have a clean stdin password mode for generic passwords.
    # This avoids shell history and config-file storage, but the setup-time process
    # arguments can be visible to the same local user for a very short time.
    subprocess.run(
        [
            "security",
...
def load_token_keychain(service: str, account: str) -> str:
    ...
    if result.returncode != 0:
        raise SystemExit(
            f"Could not read Jira API token from macOS Keychain service={service!r} account={account!r}."
        )
...
class JiraClient:
    ...
    def _url(self, path_or_url: str, params: dict[str, Any] | None = None) -> str:
        if path_or_url.startswith("http://") or path_or_url.startswith("https://"):
            ...
            # Scoped Atlassian API tokens are called through api.atlassian.com/ex/jira/{cloudId}.
            # Jira payloads still often contain site-local absolute URLs. Convert those back to
            # API paths so scoped-token mode keeps working.
    ...
    def request(
        ...
        *,
        ...
        try:
            ...
            raise JiraError(
                f"Jira API {method.upper()} {path_or_url} failed with HTTP {exc.code}: {details[:2000]}"
            ) from exc
        except urllib.error.URLError as exc:
            raise JiraError(f"Could not reach Jira API: {exc}") from exc

    def get(self, path_or_url: str, params: dict[str, Any] | None = None, *, expect_json: bool = True) -> Any:
...
def adf_to_text(node: Any, *, indent: int = 0) -> str:
    ...
    if node_type == "heading":
        ...
        return "#" * level + " " + "".join(adf_to_text(item, indent=indent) for item in content).strip()
    ...
    if node_type == "bulletList":
        ...
        for item in content:
            ...
            if item_text:
                lines.append(" " * indent + "- " + item_text.replace("\n", "\n" + " " * (indent + 2)))
        return "\n".join(lines)
    ...
    if node_type == "orderedList":
        ...
        for idx, item in enumerate(content, start=start):
            ...
            if item_text:
                prefix = f"{idx}. "
                lines.append(" " * indent + prefix + item_text.replace("\n", "\n" + " " * (indent + len(prefix))))
        return "\n".join(lines)
    ...
    if node_type == "mention":
        return (node.get("attrs") or {}).get("text") or (node.get("attrs") or {}).get("id") or "@mention"
...
def format_value(value: Any, names: dict[str, str] | None = None) -> str:
    if value is None or value == "":
        return "-"
    if isinstance(value, (str, int, float, bool)):
        return str(value)
    if isinstance(value, list):
        rendered = [format_value(item, names) for item in value]
...
def get_comments(client: JiraClient, key: str, max_comments: int) -> list[dict[str, Any]]:
    ...
    while len(comments) < max_comments:
        page = client.get(
            ...
            {"startAt": start_at, "maxResults": page_size, "orderBy": "created"},
        )
...
def get_changelog(client: JiraClient, key: str, limit: int) -> list[dict[str, Any]]:
    ...
    page = client.get(
        ...
        {"startAt": 0, "maxResults": min(max(limit, 1), 100)},
    )
...
def search_children(client: JiraClient, key: str, max_results: int) -> list[dict[str, Any]]:
    ...
    for jql in jqls:
        ...
        try:
            result = client.post("/rest/api/3/search/jql", body)
            return result.get("issues", [])
        except JiraError:
            try:
                result = client.post("/rest/api/3/search", body)
                return result.get("issues", [])
            except JiraError:
                continue
    return []
...
def is_text_attachment(att: dict[str, Any]) -> bool:
    ...
    return mime.startswith(TEXT_MIME_PREFIXES) or mime in TEXT_MIME_TYPES or suffix in TEXT_EXTENSIONS
...
def linked_issue_summary(link: dict[str, Any]) -> str:
    ...
    return f"- {direction}: {issue.get('key')} [{status}] {summary}".strip()
...
def linked_compact(issue: dict[str, Any]) -> str:
    ...
    assignee = display_user(fields.get("assignee"))
    return f"- {issue.get('key')} [{status}] ({issue_type}, {assignee}) {fields.get('summary') or ''}".strip()
...
def render_markdown(
    *,
    ...
    meta_rows = [
        ...
        ("Resolution", format_value(fields.get("resolution"))),
        ("Project", format_value(fields.get("project"))),
        ("Assignee", display_user(fields.get("assignee"))),
        ("Reporter", display_user(fields.get("reporter"))),
        ("Creator", display_user(fields.get("creator"))),
        ("Created", format_value(fields.get("created"))),
        ("Updated", format_value(fields.get("updated"))),
        ("Resolved", format_value(fields.get("resolutiondate"))),
        ("Due", format_value(fields.get("duedate"))),
        ("Components", format_value(fields.get("components"))),
        ("Labels", format_value(fields.get("labels"))),
        ("Fix versions", format_value(fields.get("fixVersions"))),
        ("Affects versions", format_value(fields.get("versions"))),
    ]
    ...
    for field_id, value in fields.items():
        if not field_id.startswith("customfield_") or value in (None, "", []):
            continue
        display = format_value(value, names)
    ...
    if attachments:
        for att in attachments:
            ...
            lines.append(
                f"- {att.get('filename')} ({att.get('mimeType') or 'unknown mime'}, {size_text}, "
                f"by {display_user(att.get('author'))}, {att.get('created') or 'unknown date'})"
            )
...
def build_payload(
    ...
    *,
    ...
    remote_links = get_remote_links(client, issue["key"])
    children = search_children(client, issue["key"], child_limit)

    attachment_previews: dict[str, str | None] = {}
    for att in (issue.get("fields") or {}).get("attachment") or []:
        attachment_previews[str(att.get("id"))] = preview_attachment(client, att, attachment_preview_bytes)
...
def cmd_setup(args: argparse.Namespace) -> int:
    site = normalize_site(args.site or input(f"Jira site [{DEFAULT_SITE}]: ").strip() or DEFAULT_SITE)
    ...
    if not auth_mode:
        auth_mode = input("Auth mode: scoped read-only token or direct site token? [scoped/direct, default scoped]: ").strip().lower() or "scoped"
    ...
    config: dict[str, Any] = {
        "site": site,
        "api_base": api_base[:-len("/rest/api/3")],
        "email": email,
        "auth_mode": auth_mode,
        "cloud_id": cloud_id,
        "token_storage": store,
...
def cmd_read(args: argparse.Namespace) -> int:
    ...
    payload = build_payload(
        ...
        changelog_limit=args.changelog,
        child_limit=args.children,
        attachment_preview_bytes=args.attachment_preview_bytes,
    )
...
def normalize_argv(argv: list[str]) -> list[str]:
    ...
    # Allow the dotfiles-style convenience call: `jira_ticket PLAN-83`.
